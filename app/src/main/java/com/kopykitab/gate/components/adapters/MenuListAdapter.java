...
import com.kopykitab.gate.components.SyncDataFromAPI;
...
public class MenuListAdapter extends BaseExpandableListAdapter implements ExpandableListView.OnChildClickListener {
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if (groupPosition > 0) {
            ...
            com.kopykitab.gate.components.TextView userName = (com.kopykitab.gate.components.TextView) convertView.findViewById(R.id.menu_list_option_text);
    ...
    public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
        ...
        if (groupPosition > 0) {
            if (groupPosition == 1) {
                int totalChild = getGroup(groupPosition).size();
                if ((totalChild - 1) == childPosition && Utils.officialCalculatorText.equals(getChild(groupPosition, childPosition).get("name"))) {
                    Utils.showOfficialCalculator(mContext);
                } else {
